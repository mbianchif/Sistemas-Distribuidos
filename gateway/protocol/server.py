...
from rabbit.mailer import Mailer
...
class Server:
    def __init__(self, config):
        ...
        self._input_copies = self._mailer.initialize()
        self._eofs_recv = defaultdict(int)
    ...
    def run(self):
        ...
        self._mailer.consume(self.handle_result, conn)
        ...
        for _ in range(3):
            ...
            while True:
                ...
                if msg.kind == MSG_FIN:
                    ...
                    self._mailer.publish_eof(filename, body)
                    break

                elif msg.kind == MSG_BATCH:
                    body = lines_to_sanitize(msg.data)
                    self._mailer.publish_batch(filename, body)
    ...
    def handle_result(self, conn: CsvTransferStream, ch, method, _properties, body):
        ...
        if kind == BATCH:
            ...
            self._eofs_recv[query] += 1
            if self._eofs_recv[query] >= self._input_copies[query]:
                eof = Eof.decode(data)
